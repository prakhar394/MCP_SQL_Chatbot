...
What refrigerator or dishwasher part information can I help you with today?"""
    ...
    async def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        messages = [
            {"role": "system", "content": """You are an appliance parts assistant specializing in refrigerator and dishwasher parts.
                ...
                - "is_in_scope": boolean (true if query is about refrigerator or dishwasher parts)
                - "needs_retrieval": boolean (true if information needs to be retrieved from database)
                
                Example response: {"is_in_scope": true, "needs_retrieval": true}"""},
            {"role": "user", "content": f"""Query: {query}
                {history_context}
                
                Is this query about refrigerator or dishwasher parts? Does it need information retrieval?
                Return only valid JSON with the two fields listed above, no other text."""}
        ]
        ...
        try:
            result = QueryAnalysis.model_validate_json(response_text)
        except Exception as e:
            logger.error(f"Failed to parse query analysis: {e}")
            logger.error(f"Response text was: {response_text}")
            # Default to in-scope and needs retrieval if parsing fails
            result = QueryAnalysis(is_in_scope=True, needs_retrieval=True)
    ...
    async def decide_batch_tools(self, query: str, previous_results: List[ToolResult] = None) -> Optional[BatchToolCall]:
        ...
        messages = [
            {"role": "system", "content": f"""You are a tool-calling assistant for an appliance parts system.
                ...
                - "tool_calls": array of objects, each with "tool_name" (string), "table_name" (string), "query" (string)
                - "should_continue": boolean (true if more tool calls needed)
                
                Example: {{"tool_calls": [{{"tool_name": "execute_read_query", "table_name": "", "query": "SELECT * FROM parts WHERE part_id = 'PS123'"}}], "should_continue": false}}"""},
            {"role": "user", "content": f"""Query: {query}
                {history_context}
                {results_context}
                
                What tools should be called next in parallel to help answer this query?
                Return only valid JSON with the structure shown above, no other text."""}
        ]
        ...
        try:
            result = BatchToolCall.model_validate_json(response_text)
        except Exception as e:
            logger.error(f"Failed to parse batch tool call: {e}")
            logger.error(f"Response text was: {response_text}")
            # Return empty batch if parsing fails
            result = BatchToolCall(tool_calls=[], should_continue=False)
    ...
    async def validate_response(self, query: str, response: str, retrieval_result: Optional[List[ToolResult]] = None) -> ResponseValidation:
        ...
        # Get JSON schema from Pydantic model for reference
        schema = ResponseValidation.model_json_schema()
        ...
        messages = [
                {"role": "system", "content": """You are a response validator for an appliance parts assistant specializing in refrigerator and dishwasher parts.
                    ...
                    You must respond with a JSON object containing these exact fields:
                    - "is_appropriate": boolean (true if response is appropriate)
                    - "stays_in_scope": boolean (true if response stays in scope)
                    - "hallucination": boolean (true if response contains hallucinations)
                    - "feedback": string or null (feedback if response needs improvement, null if no feedback needed)
                    
                    Example response: {"is_appropriate": true, "stays_in_scope": true, "hallucination": false, "feedback": null}"""},
                {"role": "user", "content": f"""Original query: {query}
                    Retrieved data: {context}
                    Generated response: {response}

                    Validate this response. Return only valid JSON with the four fields listed above, no other text."""}
        ]
        ...
        try:
            result = ResponseValidation.model_validate_json(response_text)
        except Exception as e:
            logger.error(f"Failed to parse validation response: {e}")
            logger.error(f"Response text was: {response_text}")
            # Return a default validation that passes to avoid blocking
    ...
    async def cleanup(self):
        ...
        try:
            ...
            for server_name, session in list(self.sessions.items()):
                try:
                    # Sessions will be closed when exit_stack is closed
                    pass
                except Exception as e:
                    logger.warning(f"Error closing session {server_name}: {str(e)}")
            
            # Close the exit stack (this closes all async context managers)
            await self.exit_stack.aclose()
        except Exception as e:
            # Suppress errors during cleanup - they're often due to async context issues
